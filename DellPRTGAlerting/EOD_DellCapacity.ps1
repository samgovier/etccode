...
.NOTES
    File Name       : EOD_DellCapacity.ps1
    Author          : Sam Govier
...
try {
    ...
    # Pull each volume and add a channel for percent free space
    Get-EODDellScVolume -DellApiConn $dellConn -ScSerialNumber $ScSerialNumber | ForEach-Object -Process {
        $percentFreeSpace = Get-EODDellScVolumePercentFreeSpace -DellApiConn $dellConn -VolumeInstanceId $PSItem.InstanceId
        $XMLOutput += Add-EODDellPRTGChannel -ChannelName "Volume $($PSItem.Name) % Free Space" -ChannelValue $percentFreeSpace -Unit "Percent" -Float
    }

    # Pull each tier and add a channel for percent free space
    Get-EODDellScStorageTypeTier -DellApiConn $dellConn -ScSerialNumber $ScSerialNumber | ForEach-Object -Process {
        $percentFreeSpace = Get-EODDellScTierPercentFreeSpace -DellApiConn $dellConn -TierInstanceId $PSItem.InstanceId
        $XMLOutput += Add-EODDellPRTGChannel -ChannelName "$($PSItem.DiskTier) % Free Space" -ChannelValue $percentFreeSpace -Unit "Percent" -Float
    }
}
catch {
